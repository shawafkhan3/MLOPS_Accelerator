...
def main(model_name, datastore_name,train_script, eval_script, data_prep_script,monitor_script, pipeline_name, compute_name, environment_path, pipeline_version=None):
...
#   env= Environment.from_conda_specification('conda_dependencies.yml','project_demo/mlops/configuration/environment/deduction_classification')
    ...
    # Create steps
    if data_prep_script:
        print("Entered prep script")
        data_prep_step = PythonScriptStep(
            name=f"Dataprocessing for {model_name} Model",
            source_directory="project_demo/mlops/src",
        ...
        print("Done Dataprep script")
    print("Entered train one")
    ...
    ## Data Monitor script
    monitor_step = PythonScriptStep(
        name=f"Monitor {model_name} Model",
        source_directory="project_demo/mlops/src",
        script_name=monitor_script,
        compute_target=compute_target,
        arguments=[
            '--model_name', model_name
        ],
    ...
    evaluate_step = PythonScriptStep(
        ...
        source_directory="project_demo/mlops/operations",
    ...
    if data_prep_script:
        ...
        pipeline_steps = [data_prep_step, train_step,monitor_step, evaluate_step, register_step]
    else:
        evaluate_step.run_after(train_step)
        register_step.run_after(evaluate_step)
        pipeline_steps = [train_step, monitor_step, evaluate_step, register_step]
